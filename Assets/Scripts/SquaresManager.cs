...
public class SquaresManager : MonoBehaviour
{
    ...
    private void PlaceSquares()
    {
        ...
        float cellWidth = parentRectTransform.rect.width / countX;
        float cellHeight = parentRectTransform.rect.height / countY;
        outX = 0f;               // промежуток между ячейками по оси X
        outY = 0f;               // промежуток между ячейками по оси Y
        for (int i = 0; i < transform.childCount; i++)
        {
            ...
            float posX = (column + 1) * outX + column * cellWidth - parentRectTransform.rect.width / 2f + cellWidth / 2f;

            // Рассчитываем позицию по оси Y
            float posY = parentRectTransform.rect.height / 2f - ((row + 1) * outY + row * cellHeight) - cellHeight / 2f;
